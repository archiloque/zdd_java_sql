...
public class AddressResourceTest extends AbstractResourceTest {
    ...
    public void testFindAllSomethingOldPersistence() {
        ...
        PersonEntity personEntity2 = createPersonEntity(1L, "John Doe", null);
        AddressEntity addressEntity = createAddressEntity(1L, "Near the shore", personEntity2);
        when(personDAO.findByIdWithLock(1L)).thenReturn(Optional.of(personEntity1));
        when(addressDAO.create("Near the shore", personEntity1)).thenReturn(addressEntity);
        ...
        when(addressDAO.findByPerson(personEntity2)).thenReturn(addressesEntities);
        Addresses actual = resources.client().target("/v2/people/1/addresses").request().get(Addresses.class);
        ...
        verify(personDAO).findByIdWithLock(1L);
        verify(addressDAO).create("Near the shore", personEntity1);
        verify(personDAO).update(personEntity1);
        verify(addressDAO).findByPerson(personEntity2);
    }
